...
func (r *SQLiteSavingMovementRepo) Create(m *core.SavingMovement) (int64, error) {
	res, err := r.db.Exec(
		`INSERT INTO saving_movements (account_id, type, amount_usd, amount_usdt, amount_bs, description, date, created_transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		m.AccountID, m.Type, m.AmountUsd, m.AmountUsdt, m.AmountBs, m.Description, m.Date, m.CreatedTransactionID)
	if err != nil {
...
func (r *SQLiteSavingMovementRepo) Update(m *core.SavingMovement) error {
	_, err := r.db.Exec(`UPDATE saving_movements SET amount_usd=?, amount_usdt=?, amount_bs=?, description=?, date=? WHERE id=?`,
		m.AmountUsd, m.AmountUsdt, m.AmountBs, m.Description, m.Date, m.ID)
	return err
}

func (r *SQLiteSavingMovementRepo) ListByAccount(accountID int64) ([]core.SavingMovement, error) {
	rows, err := r.db.Query(`SELECT id, account_id, type, amount_usd, amount_usdt, amount_bs, description, date, created_transaction_id, created_at FROM saving_movements WHERE account_id=? ORDER BY date DESC, created_at DESC`, accountID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.AccountID, &m.Type, &m.AmountUsd, &m.AmountUsdt, &m.AmountBs, &m.Description, &m.Date, &m.CreatedTransactionID, &m.CreatedAt); err != nil {
